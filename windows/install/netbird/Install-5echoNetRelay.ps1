...
# KONFIGURASJON
# ------------------------------------------------------------------------------
$ServiceDisplayName  = "5echo.io NetRelay"          # Navn som vises utad
$ServiceInternalName = "NetRelay"                    # Intern ID (ingen spaces)
$NetbirdVersion      = "latest"                      # "latest" eller f.eks. "0.28.0"
...
function Get-InstalledNetbirdVersion {
    ...
    if (-not (Test-Path $netbirdExe)) { return $null }
    try {
        $output = & $netbirdExe version 2>&1
...
function Stop-NetbirdProcesses {
    ...
    @("netbird", "netbird-ui", "wt-go") | ForEach-Object {
...
# ------------------------------------------------------------------------------
# DETEKSJON – Er NetBird/NetRelay allerede installert?
# ------------------------------------------------------------------------------
...
# Vis meny dersom installert og ikke allerede i en spesifikk modus
...
if (-not (Test-IsAdmin)) {
    ...
    if (-not $UpdateOnly -and -not $Uninstall) {
        $setupKeyInput = Read-Host "Skriv inn $ServiceDisplayName Setup Key"
        $setupKeyInput = $setupKeyInput.Trim()
...
if ($Uninstall) {
    ...
    Write-Log "=== $ServiceDisplayName – AVINSTALLASJON starter ==="
    ...
    # Avinstaller via MSI (finn produktkode i registry)
        ...
        Get-ChildItem $root -ErrorAction SilentlyContinue | ForEach-Object {
            ...
            if ($props.DisplayName -and ($props.DisplayName -match "(?i)netbird" -or $props.DisplayName -match "(?i)netrelay")) {
                ...
                if ($productCode -match "^\{.*\}$") {
                    Write-Log "Avinstallerer produkt: $productCode"
    ...
    # Fjern registry-nøkler (autostart, service)
    ...
    # Fjern brannmurregel for NetBird (SSH og RDP beholdes – de er OS-funksjoner)
    Write-Log "Fjerner NetBird brannmurregler..."
...
# ------------------------------------------------------------------------------
New-Item -ItemType Directory -Path $TempDir -Force | Out-Null
Write-Log "=== $ServiceDisplayName – $(if ($UpdateOnly) { 'OPPDATERING' } else { 'INSTALLASJON' }) starter ==="
Write-Log "Kjører som: $env:USERNAME | Maskin: $env:COMPUTERNAME"

# ------------------------------------------------------------------------------
# HENT SETUP KEY (kun ved installasjon)
# ------------------------------------------------------------------------------
$SetupKey = ""
if (-not $UpdateOnly) {
...
if ($UpdateOnly) {
    ...
    Write-Log "Installert versjon : $(if ($installedVer) { $installedVer } else { 'ikke funnet' })"
    Write-Log "Tilgjengelig versjon: $NetbirdVersion"
...
# Flytt netbird.exe til tilpasset mappe (skjuler produktnavn)
...
# ------------------------------------------------------------------------------
# KJØR MASKERING PÅ NYTT ETTER OPPDATERING
# (MSI-oppdatering kan tilbakestille navn i registry og service)
# ------------------------------------------------------------------------------
if ($UpdateOnly) {
    Write-Log "Kjører re-maskering etter oppdatering..."
    ...
    foreach ($root in $uninstallRoots) {
        Get-ChildItem -Path $root -ErrorAction SilentlyContinue | ForEach-Object {
            ...
            if ($displayName -and $displayName -match "(?i)netbird") {
    ...
    if (Test-Path $sourceExe) {
        ...
        Write-Log "Prosess re-maskert: netrelay.exe oppdatert."
    }
...
if ($sshProc -and $sshProc.ExitCode -eq 0) {
    Write-Log "NetBird SSH aktivert."
} else {
    # Ny syntaks i nyere versjoner
    Start-Process $NetbirdExe `
        -ArgumentList "up --allow-server-ssh" `
        -Wait -WindowStyle Hidden -ErrorAction SilentlyContinue
    Write-Log "NetBird SSH aktivert (via --allow-server-ssh)." "INFO"
}

# ------------------------------------------------------------------------------
# AKTIVER WINDOWS OPENSSH SERVER
# ------------------------------------------------------------------------------
Write-Log "Installerer og aktiverer Windows OpenSSH Server..."

# Installer OpenSSH via Windows Capability
...
if ($sshCapability -and $sshCapability.State -ne "Installed") {
    ...
    Write-Log "OpenSSH Server var allerede installert."
}

# Start og sett tjenesten til automatisk
$sshService = Get-Service -Name "sshd" -ErrorAction SilentlyContinue
if ($sshService) {
    Set-Service -Name "sshd" -StartupType Automatic
    Start-Service -Name "sshd" -ErrorAction SilentlyContinue
    Write-Log "OpenSSH Server tjeneste startet (Automatic)."
} else {
    Write-Log "sshd-tjeneste ikke funnet – OpenSSH-installasjon kan ha feilet." "WARN"
}

# Tillat SSH i Windows Firewall
$fwRuleSSH = Get-NetFirewallRule -Name "OpenSSH-Server-In-TCP" -ErrorAction SilentlyContinue
if (-not $fwRuleSSH) {
    New-NetFirewallRule -Name "OpenSSH-Server-In-TCP" `
        -DisplayName "OpenSSH SSH Server (TCP-In)" `
        -Enabled True -Direction Inbound -Protocol TCP `
        -Action Allow -LocalPort 22 | Out-Null
    Write-Log "Brannmurregel for SSH (port 22) lagt til."
} else {
    Write-Log "Brannmurregel for SSH eksisterer allerede."
}

# ------------------------------------------------------------------------------
# AKTIVER RDP (Remote Desktop Protocol)
# ------------------------------------------------------------------------------
Write-Log "Aktiverer RDP..."

# Aktiver RDP i registry
Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server" `
    -Name "fDenyTSConnections" -Value 0 -Type DWord -Force

# Sett NLA (Network Level Authentication) – anbefalt sikkerhet
Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp" `
    -Name "UserAuthentication" -Value 1 -Type DWord -Force -ErrorAction SilentlyContinue

# Start og sett Remote Desktop Services til automatisk
...
# Tillat RDP i Windows Firewall
$fwRuleRDP = Get-NetFirewallRule -DisplayName "*Remote Desktop*" -ErrorAction SilentlyContinue |
    Where-Object { $_.Direction -eq "Inbound" -and $_.Enabled -eq "True" }
if (-not $fwRuleRDP) {
    Enable-NetFirewallRule -DisplayGroup "Remote Desktop" -ErrorAction SilentlyContinue
    Write-Log "Brannmurregler for RDP aktivert."
} else {
    Write-Log "Brannmurregler for RDP var allerede aktive."
}
...
$NetbirdExeSource = "$InstallDir\netbird.exe"
if (-not (Test-Path $NetbirdExeSource)) {
    $NetbirdExeSource = "$env:ProgramFiles\Netbird\netbird.exe"
}
$MaskedExe = "$InstallDir\netrelay.exe"

if (Test-Path $NetbirdExeSource) {
    # Stopp tjeneste midlertidig før kopiering
    Stop-NetbirdProcesses
    Copy-Item $NetbirdExeSource -Destination $MaskedExe -Force -ErrorAction SilentlyContinue
...
# ------------------------------------------------------------------------------
# MASKERING – INSTALLERTE APPLIKASJONER (Innstillinger → Apper)
# ------------------------------------------------------------------------------
...
foreach ($root in $uninstallRoots) {
    Get-ChildItem -Path $root -ErrorAction SilentlyContinue | ForEach-Object {
        $displayName = (Get-ItemProperty -Path $_.PSPath -Name "DisplayName" -ErrorAction SilentlyContinue).DisplayName
        if ($displayName -and $displayName -match "(?i)netbird") {
            Write-Log "Fant oppføring: $displayName – oppdaterer til '$ServiceDisplayName'..."
            Set-ItemProperty -Path $_.PSPath -Name "DisplayName"  -Value $ServiceDisplayName  -Force
            Set-ItemProperty -Path $_.PSPath -Name "Publisher"    -Value "5echo.io"            -Force
            ...
            Write-Log "Oppføring oppdatert: $($_.PSPath)"
        }
    }
}

# ------------------------------------------------------------------------------
# MASKERING – WINDOWS SERVICE (services.msc)
# ------------------------------------------------------------------------------
Write-Log "Maskerer Windows Service visningsnavn..."
...
function Set-ServiceMasking {
    ...
    foreach ($svcName in $serviceNames) {
        ...
        if ($svc) {
            # Endre visningsnavn via sc.exe (støtter spaces i navn)
            $result = & sc.exe config $svcName displayname= "$ServiceDisplayName" 2>&1
            Write-Log "Service '$svcName' visningsnavn endret til '$ServiceDisplayName'. sc.exe: $result"

            # Endre også beskrivelse
            & sc.exe description $svcName "$ServiceDisplayName – sikker nettverkstilkobling" 2>&1 | Out-Null

            # Oppdater registry direkte som backup
...
# ------------------------------------------------------------------------------
Write-Log "Deaktiverer taskbar-ikon (netbird-ui)..."
...
Get-Process -Name "netbird-ui" -ErrorAction SilentlyContinue |
    Stop-Process -Force -ErrorAction SilentlyContinue

Write-Log "Taskbar-ikon deaktivert."

# ------------------------------------------------------------------------------
# OPPRETT SCHEDULED TASK FOR AUTOMATISK OPPDATERING (daglig)
# ------------------------------------------------------------------------------
...
# Kopier scriptet til ProgramData for bruk av scheduled task.
# Dersom scriptet kjøres direkte fra minnet (iex/irm), finnes ingen fysisk fil –
# last da ned fra GitHub i stedet.
...
if ($localScriptPath -and (Test-Path $localScriptPath)) {
    ...
    Write-Log "Script kopiert fra lokal fil til $scriptDestination"
} else {
    Write-Log "Ingen lokal scriptfil funnet – laster ned fra $ScriptPublicUrl..."
    try {
        $ProgressPreference = 'SilentlyContinue'
...
# Trigger 1: Daglig kl. 03:00
$triggerDaily = New-ScheduledTaskTrigger -Daily -At "03:00"

# Trigger 2: Ved oppstart – fanger opp maskiner som var av kl. 03:00
# Venter 3 minutter slik at nettverk er oppe og klart
$triggerBoot = New-ScheduledTaskTrigger -AtStartup
$triggerBoot.Delay = "PT3M"   # ISO 8601 – 3 minutters forsinkelse

$taskTrigger = @($triggerDaily, $triggerBoot)
...
# Fjern gammel task om den finnes
Unregister-ScheduledTask -TaskName $UpdateTaskName -Confirm:$false -ErrorAction SilentlyContinue
...
Write-Log "Scheduled Task '$UpdateTaskName' opprettet (daglig kl. 03:00 + ved oppstart som SYSTEM)."
...
Write-Host "  Auto-update   : Daglig kl. 03:00 + ved oppstart (Scheduled Task)" -ForegroundColor Cyan
Write-Host "  Logg          : $LogFile" -ForegroundColor DarkGray
Write-Host ""
